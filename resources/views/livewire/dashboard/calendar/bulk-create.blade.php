        ...
        <div class="row g-xl-4 g-lg-3 g-2 justify-content-between">
            <div class="col-xxl-12 col-xl-12 col-lg-12 order-1 order-xxl-0">
                <form wire:submit.prevent='addDate' class="card mt-3">
                    ...
                    <div class="card-body">
                        <div class="row g-3 px-5 d-none">
                            <div class="col-md-5">
                                <label class="form-label small text-muted">Start Date</label>
                                <input type="date" wire:model.lazy='start_date' class="form-control" placeholder="Date">
                                @error('start_date') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>
                            <div class="col-md-5">
                                <label class="form-label small text-muted">End Date</label>
                                <input type="date" wire:model.lazy='end_date' class="form-control" placeholder="Date">
                                @error('end_date') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>
                        </div>
                        <div class="row g-3 px-5 mb-1">
                            @foreach ($days as $i => $day)
                                <div class="col">
                                    <label class="custom-control custom-checkbox">
                                    <input type="checkbox" wire:model="days.{{$i}}" class="custom-control-input">
                                    <span class="custom-control-indicator">{{ $i }}</span>
                                    </label>
                                </div>
                            @endforeach
